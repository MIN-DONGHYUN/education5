...
public class FileManager {
	...
	// 리턴값 : 서버에 저장된 새로운 파일명(예: 2023051909125435243254235235234.png)
	public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {
		...
		String fileExt = originalFilename.substring(originalFilename.lastIndexOf(".")); // 마지막 . 위치값 알아오기
		...
		newFilename = String.format("%1$tY%1$tm%1$td%1$tH%1$tM%1$tS", Calendar.getInstance());  // %1$tY%1$tm%1$td%1$tH%1$tM%1$tS 이것은 원,월,일, 시,분,초까지 알아오는 것이다. 
		newFilename += System.nanoTime();
		newFilename += fileExt;
		// newFilename 은 예를 들면, 2023051909125435243254235235234.png 와 같이 된다.   
		...
		if(!dir.exists()) {
			...
			dir.mkdirs(); // 파일을 저장할 경로인 폴더를 생성한다. mk 가 make를 뜻한다 즉 폴더를 생성함 
		}
		...
		// pathname 은 예를 들면, C:\NCS\workspace(spring)\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\Board\resources\files\2023051909125435243254235235234.png 이다.      
