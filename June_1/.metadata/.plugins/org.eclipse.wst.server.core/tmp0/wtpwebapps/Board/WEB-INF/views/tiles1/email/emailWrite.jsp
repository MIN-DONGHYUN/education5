...
<script type="text/javascript">
  ...
  $(document).ready(function(){
	  ...
	  $(document).on("click", "span.delete", function(e){
		...
		// 배열에서 파일을 제거하기 전 : [object File],[object File],[object File]
		
		file_arr.splice(idx,1);
		//console.log("배열에서 파일을 제거한 후 : " + file_arr);
		// 배열에서 파일을 제거한 후 : [object File],[object File]
		<%-- 
		...
		$(e.target).parent().remove();   // 정상적으로 파일을 제거함 (배열에서 삭제)
	  });
       ...
       <%-- ==== // 글내용 유효성 검사(스마트 에디터 사용 할 경우) 끝 ==== // --%>  
		   ...
		   // 또는
	   	   ...
	   	   if(file_arr.length > 0) {//파일 첨부가 있을 경우
	   		   ...
	   		   // 첨부한 파일의 총 합의 크기가 10MB 이상이라면 메일 전송을 못하게 막는다.
	   		   let sum_file_size = 0;
	   	   	   ...
	   	   	   console.log(sum_file_size);
	   	   	   
	   	   	   // 첨부한 파일의 총합이 10MB 이상이라면 
	   	   	   if( sum_file_size >= 10*1024*1024 ) {
	   	   		   alert("첨부한 파일의 총합의 크기가 10MB 이상이라서 파일을 업로드 할 수 없습니다.!!");
	   	   	   	   return;    // 종료 
	   	   	   
	   	   	   }
	   	   	   else {  //10MB 미만이라면   //formData 속에 첨부파일 넣어주기 
	   	...
	   	$.ajax({
            ...
            success:function(json){
               // console.log("~~~ 확인용 : " + JSON.stringify(json));
                // ~~~ 확인용 : {"result":1}
                if(json.result == 1) {
                   location.href = "<%= ctxPath%>/emailWrite_done.action";   /* 다른 action을 호출 */
                }
                else {
                   alert("메일보내기가 실패했습니다.ㅜㅜ");
                }
            },
