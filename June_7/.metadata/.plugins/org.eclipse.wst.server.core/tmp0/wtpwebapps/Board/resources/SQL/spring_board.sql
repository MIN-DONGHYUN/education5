...
-- 특정부서명 근무하는 직원들에 대해서 성별에 따른 인원수 및 퍼센티지 알아오기 --
select func_gender(jubun) AS gender
      , count(*) AS cnt
      , round(count(*)/(select count(*) from employees)*100, 2) AS percentage
from employees E left join departments D
on E.department_id = D.department_id
where D.department_name = '부서없음'
group by func_gender(jubun)
order by gender;
...
select userid, email
from tbl_member
where userid in('mdh2057','leess');

update tbl_member set email = (select email from tbl_member where userid = 'mdh2057')
where userid = 'leess';
...
values(seq_reservation.nextval, 'mdh2057', to_date('2023-06-03 13:00','yyyy-mm-dd hh24:mi') );

insert into tbl_reservation(reservationSeq, fk_userid, reservationDate)
values(seq_reservation.nextval, 'leess', to_date('2023-06-03 14:00','yyyy-mm-dd hh24:mi') );

insert into tbl_reservation(reservationSeq, fk_userid, reservationDate)
values(seq_reservation.nextval, 'smdh2057', to_date('2023-06-04 11:00','yyyy-mm-dd hh24:mi') );
